...
<body>
		...
		<div class="container">
			...
			<a class="blog-nav-item <#if 0==g_folderId>active</#if>" href="${BASE_PATH}/index.htm">首页</a>
	                <#list tag_folder_list as tag_folder>
	                	<a class="blog-nav-item <#if tag_folder.folderId==g_folderId>active</#if>" href="<@xm_folder_url_tag folderId=tag_folder.folderId/>">${tag_folder.name}</a>
			</#list>
           		</@xm_folder_list_tag>
			</nav>
		</div>
	...
	<div class="container">
		<div class="blog-header">
			<h1 class="blog-title">${xm_seo_title}</h1>
			<p class="lead blog-description">${xm_seo_description}</p>
		</div>