...
public class TemplateService {
	...
	//private static String FOLDER_TEMPLATE_PREFIX = "list";
	private static String FILE_TEMPLATE_PREFIX = "article";
	//private static String FILE_TEMPLATE_PREFIX = "detail";
	private static String CONTACT_TEMPLATE_PREFIX = "contact";
	 ...
	 * @throws FolderNotFoundException
	 */
	public String getContactTemplate(long folderId, long articleId)
			throws TemplateNotFoundException, FolderNotFoundException {
		List<FolderVo> folderPathList = folderService
				.getFolderPathListByFolderId(folderId);
		List<String> themeOrderList = new ArrayList<String>();
		themeOrderList.add(CONTACT_TEMPLATE_PREFIX);
		String themeString = CONTACT_TEMPLATE_PREFIX;
		for (Folder folder : folderPathList) {
			themeString = themeString + "-" + folder.getEname();
			themeOrderList.add(themeString);
		}
		...
		throw new TemplateNotFoundException("模板文件："
				+ this.getTemplatePath(CONTACT_TEMPLATE_PREFIX) + " 不存在！！");
	}
	...
	private String getTemplatePath(String template) {
		return "/template/"
				+ configService.getStringByKey(ConfigConstant.XM_TEMPLATE)
				+ "/" + template;
	}
	...
	public Boolean isExist(String theme) {
		String themePath = "/WEB-INF/static/template/"
				+ configService.getStringByKey(ConfigConstant.XM_TEMPLATE)
				+ "/" + theme + ".ftl";
		File file = new File(SystemConstant.XM_CMS_ROOT + themePath);
		if (file.exists()) {
